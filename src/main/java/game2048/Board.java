...
class Board extends Group {
    ...
    private static final double BASELINE_OFFSET_SAME_AS_WIDTH = Double.NEGATIVE_INFINITY;

    private static final double BASELINE_OFFSET_SAME_AS_HEIGHT = Double.NEGATIVE_INFINITY;
    ...
    Board() {
        int borderSize = CELL_SIZE * GridOperator.GRID_SIZE + BORDER_WIDTH * 2;
        gridWidth = borderSize;
        gridHeight = borderSize;
    ...
    private void createScore() {
        ...
        timer=new Timeline(new KeyFrame(Duration.ZERO,
                e-> clock.set(LocalTime.now().minusNanos(time.toNanoOfDay()).format(fmt))),new KeyFrame(Duration.seconds(1)));
    ...
    private void createGrid() {
        ...
        hBottom.setMinSize(gridWidth, gridHeight);
        hBottom.setPrefSize(gridWidth, gridHeight);
        hBottom.setMaxSize(gridWidth, gridHeight);
    ...
    private class Overlay implements ChangeListener<Boolean> {
        ...
        Overlay(String message, String warning, Button btn1, Button btn2, String style1, String style2, boolean pause){
    ...
    private void initGameProperties() {
        ...
        overlay.setMinSize(gridWidth, gridHeight);
        ...
        buttonsOverlay.setMinSize(gridWidth, gridHeight / 2.);
        ...
        gameAboutProperty.addListener((observable, oldValue, newValue) -> {
            if (newValue) {
                ...
                flow.setMinSize(gridWidth, gridHeight);
                flow.setPrefSize(gridWidth, gridHeight);
                flow.setMaxSize(gridWidth, gridHeight);
                flow.setPrefSize(BASELINE_OFFSET_SAME_AS_WIDTH, BASELINE_OFFSET_SAME_AS_HEIGHT);
    ...
    void addTile(Tile tile){
        double layoutX = tile.getLocation().getLayoutX() - (tile.getMinWidth() / 2);
        double layoutY = tile.getLocation().getLayoutY() - (tile.getMinHeight() / 2);
    ...
    BooleanProperty clearGameProperty() {
        return clearGame;
    }
    
    BooleanProperty saveGameProperty() {
        return saveGame;
    }
    
    BooleanProperty restoreGameProperty() {
        return restoreGame;
    }
    ...
    */
    void saveSession(Map<Location, Tile> gameGrid) {
        saveGame.set(false);
    ...
    Once we have confirmation
    */
    boolean restoreSession(Map<Location, Tile> gameGrid) {
        timerPause.stop();
        restoreGame.set(false);
        doClearGame();
        timer.stop();
        StringProperty time = new SimpleStringProperty("");
        int score = sessionManager.restoreSession(gameGrid, time);
        if (score >= 0) {
            ...
            if(!time.get().isEmpty()) {
                this.time = LocalTime.now().minusNanos(Long.valueOf(time.get()));
            }
            timer.play();
            return true;
        } 
    ...
    void saveRecord() {
        sessionManager.saveRecord(gameScoreProperty.getValue());
    }
    
    private void restoreRecord() {
        gameBestProperty.set(sessionManager.restoreRecord());
    }
